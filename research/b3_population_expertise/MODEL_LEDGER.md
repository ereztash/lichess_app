...
| 29 | 2026-09-02 04:05 | FINAL analysis | Opus 5 | run the frozen pipeline once | read/write | `beta` = +0.01342 [+0.01243, +0.01431], reproduced out of sample in both later periods; Metric B null [one instrument read four ways plus a degenerate matched form; Gate 3 F-N4] | `results/analysis_final.json` |
...
| 32 | 2026-09-02 04:25 | **C3 repair, pinned** | Opus 5 | apply the adversary's one-line repair to the C3 null, on the three periods only | read/write, byte-identity asserted outside C3 | verdict moves `INVALID_EXPERIMENT` -> `GENERAL_REGULARITY_ONLY` (level 3) with the same seven failed H2 conditions; everything outside C3 identical (sha256 `d2794b40...`) | `src/repair_c3.py`, `results/analysis_repaired.json`, `results/verdict_repaired.json`, `results/c3_repair_diff.json`, amendment A7 |
| 33 | 2026-09-02 04:35 | Report diagnostics | Opus 5 | compute every decomposition Gate 3 requires the report to carry | read/write | capped-outcome `beta`, the engine-best-move split, the Metric B cancellation table, the detectable spread, the matched composition, the per-band residual means | `src/report_diagnostics.py`, `results/report_diagnostics.json` |
