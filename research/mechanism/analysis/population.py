...
For a frozen region R:
  ...
  2. population residual contrast under a baseline fit ON THE POPULATION (so "generic difficulty"
     is defined by the population, not by the focal player);
  3. the FOCAL player's residual elevation in R under the POPULATION baseline, on VALIDATE (and TEST
     when opened): the quantity that says "more than a same-rating player in the same situation";
  4. the distribution of per-player-side elevations in R across the population (one number per
     side with >= 8 decisions in R and >= 8 out), and the focal player's rank in it.
Population evidence establishes only what the region means for a population; it cannot establish
that the focal player has the mechanism (mission §3 R6).
"""
...
from common import load_decisions, eligible, chronological_split, region_contrast, clustered_rate_se, AUTO_CORPUS
from focal import exclude_focal, leakage_report
...
def main():
    ...
    a = ap.parse_args()
    design = vocab.DESIGN
    me = chronological_split(eligible(load_decisions(a.decisions, corpus=a.corpus or AUTO_CORPUS)), design["derive_frac"], design["validate_frac"])
    focal_corpus = str(me["corpus"].iloc[0]) if "corpus" in me.columns and len(me) else None
    focal_keys = set(me["player_key"].dropna().unique()) if "player_key" in me.columns else set()
    if a.focal_player_key:
        focal_keys.add(a.focal_player_key)
    pop_all = eligible(load_decisions(a.population, corpus=None))
    leak = leakage_report(pop_all, focal_corpus, focal_keys)
    pop = exclude_focal(pop_all, focal_corpus, focal_keys).reset_index(drop=True)
    ...
    out = {"region": a.region, "frame": a.frame,
           ...
           "focal": {"corpus": focal_corpus, "player_keys": sorted(str(k) for k in focal_keys),
                     "n": len(fr), "raw": raw_me, "resid_under_population_baseline": res_me},
           "leakage_guard": leak,
           "per_side": {"n_sides": len(elev), "elev_mean": float(ev.mean()) if len(ev) else None, "elev_sd": float(ev.std()) if len(ev) else None,
                        ...
                        "focal_elev": my_elev, "focal_percentile": float((ev < my_elev).mean()) if len(ev) else None,
                        "raw_q": {q: float(np.quantile(rv, q)) for q in (0.05, 0.25, 0.5, 0.75, 0.95)} if len(rv) else None,
                        "focal_raw_elev": my_raw, "focal_raw_percentile": float((rv < my_raw).mean()) if len(rv) else None}}
    ...
    print(f"focal [{a.frame}]: raw in/out {raw_me['p_in']:.3f}/{raw_me['p_out']:.3f} diff={raw_me['diff']:+.3f} z={raw_me['z']:+.2f}; resid (pop baseline) diff={res_me['diff']:+.4f} z={res_me['z']}")
    if len(ev):
        print(f"per-side elevation: n={len(ev)} mean={ev.mean():+.4f} sd={ev.std():.4f} q05/50/95={np.quantile(ev,.05):+.3f}/{np.quantile(ev,.5):+.3f}/{np.quantile(ev,.95):+.3f}; focal={my_elev:+.4f} percentile={(ev<my_elev).mean():.3f}")
