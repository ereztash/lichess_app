...
| [D28](D28-adaptive-policy-architecture.md) | may the expression of the recursive loop vary by user, context, burden and horizon? | `DEFER` | not premature — **unanalysable**: an adaptive arm varies within a build and within a player, and the record has one within-build axis and no within-player axis at all. One class is refused outright, because a prompt rate that responds to skipping curates the sample on the measured variable. `DOCUMENT_ONLY`, plus the arm that was stamped and unread becoming a stratum axis |
| [D29](D29-question-ownership.md) | may a finding the instrument produced become the player's next project without the player saying so? | `DEFER` | no. The player's own hypotheses have carried `retired` from the beginning and the instrument's carried nothing, so a question the player did not want could only be got rid of by answering it. One grade, no `Question` entity, no acceptance record and no `defer` state — acceptance is already the started drill, and "not now" licenses the same next action as silence. **Cannot merge before the FIELD run ends:** the freeze is on the bundle, and `shared/claim-state.ts` is in it |
