...
**A precondition on this gate was found after it was written, and it fails on RC-06.** A minimal
functional twin flips the trigger; if `B` is defined differently on the two sides of that flip, the
contrast measures the predicate rather than the trigger. `_threat_satisfies` is the only predicate of
the twelve screened that recomputes the trigger — a hit is *"no mate in one"*, a false alarm is *"no
check at all"* — so **Gate B cannot be run on RC-06 as specified**, and symmetrising is not available
because `B` is trivially true whenever the threat is absent. Either the twin design fixes one
predicate across both sides, or Gate B moves to a method-shaped rule class.

The same finding removes a quantity this decision had been reasoning with: **the SDT criterion here
is not a player parameter.** A move-blind agent scores *d′* **0.80** and *c* **+0.88** on RC-06 from
predicate sizes alone; across the twelve classes move-blind *c* predicts observed *c* at **r = +0.72**;
and `RC-09` vs `RC-11` — same trigger, same corpus, same noise cell, outcome versus method — moves
*c* by **+0.524** on the same players and the same positions. See
[H22/H23](../learning-v2/FALSIFICATION_REGISTER.md#h22) and
[`CRITERION_CHANNEL.md`](../learning-v2/CRITERION_CHANNEL.md); arithmetic in
[`criterion_channel.py`](../../research/learning/criterion_channel.py).

**H17's headroom finding is unaffected** — corrected for the move-blind floor, sensitivity still
orders the rating bands, 0.380 → 0.866.
