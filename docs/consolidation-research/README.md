...
falsify it found **seven methodological defects and ten new contradictions**, eight of the ten inside
...
What makes it `PARTIAL` rather than `STRONG`: **six critical questions with no authority at all**
— rollback, observability, retention, supported runtimes, who may deploy, dependency upgrades: the
whole operational surface — and a study that quoted **26 of the 204** implementation files the
repository's own governance names.
...
| [`AUTHORITY_MAP_V2_ATTACK.md`](AUTHORITY_MAP_V2_ATTACK.md) | two rounds of the completeness attack. Twelve omitted questions, six with no authority at all. **25 / 36, not 24 / 24 — and a lower bound** |
...
| [`ADVERSARIAL_REVIEW.md`](ADVERSARIAL_REVIEW.md) | nineteen attacks. Eleven on the model, eight on the repairs; between them they downgraded seven things and upgraded nothing |
...
| [`d1b_population.py`](d1b_population.py) · [`.json`](d1b_population.json) | `D1b`'s denominator and numerator, derived from the tree so the study cannot choose either |
| [`LAW_SUPPORT.json`](LAW_SUPPORT.json) | the evidence behind §B's four count columns |
