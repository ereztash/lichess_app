...
| 6 | **G-07** | a standing test at the rung the claim needs — `tests/layout/*` refuses to skip, `content-security-policy.layout.test.ts` serves `dist/public` under `vercel.json`'s exact policy | `L6 = 0`; the run that found the CSP/worker defect was a throwaway script and *"does not re-run"* | 1 | 2 | 1.00 | 0.500 | **1.00** |
