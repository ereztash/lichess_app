...
הבקרות שנרשמו מראש אומרות שההפרדה הזאת היא **סוג העמדה**, לא הזמן: כשמערבבים את התוצאה בתוך שלב
ומעמד, **28.5%** מהערבובים עדיין עוברים את הרף (מול 3.5% מכויל), וההפרדה קורסת ב**כל שמונת** התאים
שבהם אפשר בכלל להשוות. לכן **שום סף לא זז**. [`_RESULTS`](docs/research/TIME_REPRESENTATION_RESULTS.md).
