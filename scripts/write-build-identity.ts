...
import { MEASUREMENT_PROTOCOL_VERSION, type BuildIdentity } from "../shared/build-identity";
...
function gitSha(): string {
  const fromPlatform = process.env.VERCEL_GIT_COMMIT_SHA ?? process.env.GITHUB_SHA;
...
function target(): string {
  return process.env.VERCEL_ENV ?? (process.env.CI ? "ci" : "local");
}
