...
const rate = (weak: string) => {
  ...
  for (let seed = 5000; seed < 5400; seed += 1) {
    const selection = selectClaim(detect(oneWeakPhase(240, seededRandom(seed), weak)), {
      claim_id: "c",
...
describe("what the claim still is", () => {
  it("carries the scope and the n it was derived from", () => {
    const selection = selectClaim(detect(oneWeakPhase(240, seededRandom(5000), "endgame")), {
      claim_id: "c",
      created_at: "2026-01-01T00:00:00.000Z",
    })!;
  ...
  it("says nothing when nothing separated", () => {
    expect(selectClaim([], { claim_id: "c", created_at: "2026-01-01T00:00:00.000Z" })).toBeNull();
  });
});
