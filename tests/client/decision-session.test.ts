...
describe("an incomplete decision is not recordable", () => {
  ...
  it("puts both the tapped and the typed part on the record", () => {
    ...
    const event = buildCommitEvent("d-2", POSITION, draft, 9, "per-decision", "context-ribbon-visible");
  ...
  it("keeps a long selection inside the atom's 200-character bound", () => {
    ...
    const event = buildCommitEvent("d-3", POSITION, draft, 9, "per-decision", "context-ribbon-visible");
    expect(event.known.length).toBeLessThanOrEqual(200);
  });

  it("refuses to build an event from an incomplete draft", () => {
    expect(() => buildCommitEvent("id", POSITION, emptyDraft(), 5, "per-decision", "context-ribbon-visible")).toThrow(/not committable/);
  });
...
describe("the commit event is the atom, unchanged", () => {
  const event = buildCommitEvent("d-1", POSITION, complete(), 12.4, "per-decision", "context-ribbon-visible");
